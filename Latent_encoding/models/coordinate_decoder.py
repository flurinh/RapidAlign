# Latent_encoding/models/coordinate_decoder.py
"""State-space iterative decoder with slot cross-attention.

This decoder refines node states (positional encodings) iteratively. Instead of
projecting slots to a global context, each node attends to the slots via
cross-attention, allowing nodes to query the structural information they need.

Key insight: Slots should encode different structural aspects (local geometry,
global shape, connectivity, etc.), and nodes should be able to selectively
attend to relevant aspects during refinement.

Architecture per step:
  1. Project states → xyz proxy (for kNN edge construction)
  2. Build dynamic kNN edges from xyz proxy
  3. Cross-attention: nodes query slots for structural context
  4. Message-pass on states with slot context + time embedding
  5. Update states (residual)

Final output: project states → xyz coordinates.
"""
